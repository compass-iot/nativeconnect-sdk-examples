...
import {RealtimeStitchedPointsByVinsRequestSchema
...
const request = create(RealtimeStitchedPointsByVinsRequestSchema, {
  vins: ["LRW3F7FS9PC893577", "1C4RJHKG2P8863343"],
  maxStalenessMinutes: 3n,
})

for await (const response of retryStream(() => client.realtimeStitchedPointsByVins(request, { timeoutMs: TIMEOUT_MS }))) {
